...
function GuestQrDialog({
  ...
  // Canonical event-scoped check-in URL so the QR opens the right workspace.
  // (Older printed QRs with /admin/check-in?token=… still resolve via the
  // legacy redirect, which preserves the token query string.)
  const { selectedEventId } = useEvents();
  const url =
    typeof window !== "undefined"
      ? `${window.location.origin}/admin/e/${selectedEventId}/check-in?token=${rsvp.check_in_token}`
      : "";
